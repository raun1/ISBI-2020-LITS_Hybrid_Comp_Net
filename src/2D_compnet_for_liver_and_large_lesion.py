...
def normalize_0_1(x):
  minimum_val=np.amin(x)
  maximum_val=np.amax(x)
  factor=1/minimum_val
  x=x*factor
  return x
#Dice coeff
smooth = 1.
...
def sp_dice_coef_loss(y_true, y_pred):
    #y_true=tf.split(y_true, 2,axis=1)
    #tf.Print(tf.shape(y_true),[tf.shape(y_true)])

    #copy=tf.identify(y_true)
    #hi_rand=np.random.randint(10000)
    #cv2.imwrite("/home/rdey/liver_stuff/check_tensor_split/liver/"+str(hi_rand)+".png",K.eval(y_true[0])*255.0)
    #cv2.imwrite("/home/rdey/liver_stuff/check_tensor_split/liver/"+str(hi_rand)+".png",K.eval(y_true[1])*255.0)

    #return (-dice_coef(y_true, y_pred)+(10*dice_coef(tf.subtract(1.0,y_true),y_pred)))
    ...
    #MSE( (1-Seg_liver) * I_pred ) * MSE (I_true - I_pred)
    #return(keras.losses.mean_squared_error(tf.subtract(1.0,thresholded_true)*thresholded_pred,0.0)*keras.losses.mean_squared_error(y_true,y_pred))
    #return((keras.losses.mean_squared_error(tf.subtract(1.0,thresholded_true)*y_pred,0.0)+keras.losses.mean_squared_error(y_true,y_pred))/2.0)
    first_part=(tf.reduce_sum(tf.square(tf.subtract(1.0,thresholded_true)*y_pred)))/(tf.reduce_sum(tf.subtract(1.0,thresholded_true)))
    second_part=(tf.reduce_sum(tf.square(thresholded_true*tf.subtract(y_pred,y_true)))/(tf.reduce_sum(thresholded_true)+0.0001))
    return(first_part+second_part)
    #return((keras.losses.mean_squared_error(tf.subtract(1.0,thresholded_true)*y_pred,0.0)+keras.losses.mean_squared_error(y_true,y_pred))/2.0)
    #fix the domain

    #return ((K.mean(K.square(y_pred - y_true), axis=-1))*(1-dice_coef(tf.subtract(1.0,thresholded_true),thresholded_pred)))
    #return (10*keras.losses.mean_squared_error(y_true[0], y_pred))
    #dear Yi, I will change the second dice now. Sorry it will take a few searches to implement it in ternsorflow since i am not good with it
...
model=Comp_U_Net(input_shape=(512,512,1))
